...
- A Redis master (`redis-master`) using `redis:7.2`, mapped to host port `8000`.
- A Redis slave (`redis-slave-1`) using `redis:7.2`, mapped to host port `8001` and configured to replicate from `redis-master`.
- A 3-node Redis Cluster (`redis-cluster-7000`, `redis-cluster-7001`, `redis-cluster-7002`) using `bitnami/redis-cluster:7` (auto-bootstrap).
...
- If you prefer using the official `redis:7` image for cluster nodes, additional init commands (`redis-cli --cluster create ...`) are required; the Bitnami image here is chosen for easier bootstrap.
